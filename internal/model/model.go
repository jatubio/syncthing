...
func (m *Model) FolderError(id string) string {
	...
	if err == nil {
		if folder.Invalid != "" {
			l.Infof("Starting folder %q after error %v", folder.ID, folder.Invalid)
	...
	if folder.Invalid == "" {
		...
		l.Infof("Folder %q error changed: %v -> %v", folder.Invalid, err.Error())
	}
